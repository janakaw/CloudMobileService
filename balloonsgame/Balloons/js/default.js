...
(function () {
    ...
    //Menu    
    var MENUBG = "/images/background_1.png";
    var MENUNUM = 0;
    ...
    function initialize() {
        ...
        var client = new WindowsAzure.MobileServiceClient("https://baloon.azure-mobile.net/",
            "tlXUUDwKVvZzZmfHytMdQhGAxspVMz96");
    ...
    function insertProfiles(id, key, authService) {

        var today = new Date();
        var item = {
            name: id, mobileAuthenticationToken: key, pushId: "",
            authService: authService, platform: "windows", profileDate: today
        };

        client.getTable("profiles").insert(item).done(function (results) {
            client.getTable("profiles").where().read().done(function (results) {
                txtPlayerName.innerHTML = results[0].displayname;
                imgPlayer.src = results[0].avatar;
            });
        });
    }
    ...
    function insertScore() {
        var today = new Date();
        var item = { name: userId, score: score, level: lvlCurrent, scoredate: today };

        client.getTable("scores").insert(item);
